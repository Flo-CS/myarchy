...
pub(super) fn extend(layout: &mut Layout, anchor_desc: &str, direction: Direction) {
...
pub(super) fn mirror(layout: &mut Layout, anchor_desc: &str) {
    for (desc, screen) in &mut layout.screens {
...
pub(super) fn only(layout: &mut Layout, keep_desc: &str) {
    for (desc, screen) in &mut layout.screens {
        if desc == keep_desc {
            screen.state = State::On;
            screen.placement.position = Position::At(Point { x: 0, y: 0 });
...
pub(super) fn set_mode(layout: &mut Layout, desc: &str, mode: Mode) -> Result<()> {
    layout.require(desc)?.placement.mode = mode;
    Ok(())
}

pub(super) fn set_scale(layout: &mut Layout, desc: &str, scale: Scale) -> Result<()> {
...
pub(super) fn enable(layout: &mut Layout, desc: &str) -> Result<()> {
...
pub(super) fn disable(layout: &mut Layout, desc: &str) -> Result<()> {
...
mod tests {
    use super::*;
    use crate::display::layout::fixtures::{BOE, DELL, LG};
    use crate::display::layout::Screen;
