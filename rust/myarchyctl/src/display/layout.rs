...
impl Side {
    ...
    pub(crate) fn before_reference(self) -> bool {
...
impl Mode {
    fn size(self) -> Option<(i64, i64)> {
        match self {
            Self::Preferred => None,
            Self::Fixed(resolution) => Some((resolution.width, resolution.height)),
        }
    }
}
...
impl Scale {
    pub(crate) fn factor(value: f64) -> Self {
        Self::Factor((value * SCALE_DECIMALS).round() / SCALE_DECIMALS)
    }

    fn value(self) -> Option<f64> {
        match self {
            Self::Auto => None,
            Self::Factor(v) if v > 0.0 => Some(v),
            Self::Factor(_) => None,
        }
    }
}
...
impl Placement {
    ...
    pub(crate) fn logical_size(&self) -> Option<(i64, i64)> {
...
pub(crate) struct Layout {
    ...
    #[serde(default)]
    pub(crate) screens: BTreeMap<String, Screen>,
}

impl Layout {
    pub(crate) fn observe(monitors: &[Monitor]) -> Self {
    ...
    pub(crate) fn sync(&mut self, monitors: &[Monitor]) {
        self.screens
        ...
        for monitor in monitors {
            let screen = self
                .screens
                .entry(monitor.description().to_string())
        ...
        if let Some(anchor) = &self.anchor {
            if !self.screens.contains_key(anchor) {
                self.anchor = None;
            }
        }
    }
    ...
    pub(crate) fn matches(&self, monitors: &[Monitor]) -> bool {
    ...
    pub(crate) fn on(&self) -> impl Iterator<Item = (&String, &Screen)> {
    ...
    pub(crate) fn require(&mut self, desc: &str) -> Result<&mut Screen> {
        match self.screens.get_mut(desc) {
            Some(screen) => Ok(screen),
            None => bail!("no stored screen for {desc}"),
        }
    }
}
...
pub(crate) mod fixtures {
    ...
    pub(crate) const BOE: &str = "BOE 0x08B9";
    pub(crate) const LG: &str = "LG Electronics LG HDR WQHD 303NTZN51357";
    pub(crate) const DELL: &str = "Dell Inc. DELL U2412M PMSXXXX";

    pub(crate) fn monitor(name: &str, description: &str, mode: &str, at: (i64, i64)) -> Monitor {
        Monitor {
            name: name.into(),
    ...
    pub(crate) fn switched_off(mut monitor: Monitor) -> Monitor {
        monitor.disabled = true;
        monitor.resolution = Resolution::new(0, 0, 0.0);
        monitor.x = 0;
        monitor.y = 0;
        monitor.scale = 0.0;
        monitor
    ...
    pub(crate) fn laptop() -> Monitor {
        monitor("eDP-1", BOE, "1920x1080@60.003", (0, 0))
    }

    pub(crate) fn ultrawide() -> Monitor {
