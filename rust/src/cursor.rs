...
use crate::backend::cursor::Cursorctl;
...
pub(crate) fn set(cursor: &dyn Cursorctl, name: &str, size: i64) -> Result<()> {
    let applied = cursor.set(name, size);
    save_preferred(name, size)?;
    applied
}

pub(crate) fn apply_preferred(cursor: &dyn Cursorctl) -> Result<()> {
...
pub(crate) fn reset(cursor: &dyn Cursorctl) -> Result<()> {
    let _ = fs::remove_file(state_file());
    apply_preferred(cursor)?;
    theme::render()
}
