...
fn connect() -> Result<UnixStream> {
    match ipc::connect() {
        ...
        {
            Err(anyhow::Error::new(e).context(UserError::IdleDaemonNotRunning))
        }
        Err(e) => Err(e).context("failed to connect to myarchy-idle-inhibitor daemon"),
    }
}
